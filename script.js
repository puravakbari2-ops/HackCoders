...
// All 432 real schemes loaded from backend/data/schemes.json
let ALL_SCHEMES = [
    {
        "id": "1",
...
nextBtn.addEventListener('click', () => {
    if (currentStep < FORM_STEPS.length - 1) {
        ...
        // Submit - show results with personalized scoring
        closeModal();
        applyProfileScoring();
        showResults();
    }
...
function applyProfileScoring() {
    ...
    ALL_SCHEMES.forEach(scheme => {
        ...
        scheme.baseMatchScore = Math.max(matchPercent, 50);
    });

    // Sort ALL_SCHEMES descending by score
    ALL_SCHEMES.sort((a, b) => (b.baseMatchScore || 0) - (a.baseMatchScore || 0));
}
...
function renderResults(schemes) {
    ...
    summary.innerHTML = `
        ...
        <div class="results-filters">
            <div class="filter-chip ${currentResultsFilter==='all'?'active':''}" data-filter="all">All Schemes</div>
    ...
    if (schemes.length === 0) {
        grid.innerHTML = `
            <div style="grid-column:1/-1; text-align:center; padding:60px 20px; color:var(--text-secondary);">
                ...
                <div>Try a different keyword like <em>"kisan"</em>, <em>"scholarship"</em>, <em>"housing"</em>, or <em>"pension"</em></div>
            </div>`;
        return;
    }
    ...
    grid.innerHTML = schemes.map(scheme => `
        <div class="result-card">
            ...
            <div class="result-card-footer">
                <a href="${scheme.applyLink || '#'}" target="_blank" rel="noopener noreferrer" class="result-card-link">Apply Now <i class="fas fa-external-link-alt"></i></a>
                <div class="result-eligibility"><i class="fas fa-check-circle"></i> ${scheme.baseMatchScore || '—'}% Match</div>
            </div>
        </div>
    `).join('');
}
...
function handleSearch(input) {
    ...
    showResults(query);   // pass query — works even if empty (shows all)
}
